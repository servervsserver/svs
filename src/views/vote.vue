<template>
  ...
  <div v-else-if="pool.length > 1 && hasvoted == false">
    <h1>We detected that you have participated in multiple servers</h1>
    <h2>To continue, select the Server you would like to vote on behalf of:</h2>
    <ChoosePool
...
export default {
  ...
  updated() {
    if (typeof this.$svsAuth.user.id != "undefined") {
      ...
      get(child(dbRef, `realTimeVoting/voters`))
        .then((snapshot) => {
          console.log("here");
          if (snapshot.exists()) {
            var foreachresults = [];
            Object.values(snapshot.val()).forEach((element) => {
              foreachresults.push(
                bcrypt.compareSync(this.$svsAuth.user.id, element)
              );
            });
  ...
  mounted() {
    if (this.$data.pool.length < 1) {
      this.$data.pool = ["community"];
    }
  },
  methods: {
    validateVoteData: function (_data_, availableOptions) {
      ...
      if (returnvariable == false) {
        this.$data.VoteValidationErrorMessage =
      ...
      if (returnvariable == false) {
        this.$data.VoteValidationErrorMessage =
          "* You cannot vote for the same EP twice";
        return false;
      }

      if (_data_[0] != true) {
        this.$data.VoteValidationErrorMessage =
          "* Please agree to the Terms of Voting and Code of Conduct";
        returnvariable = false;
      }
    ...
    submitvote: function () {
      ...
      if (
        this.validateVoteData(voteData, this.$data.EPs) &&
        this.$data.hasvoted == false
      ) {
        push(
          child(dbRef, `realTimeVoting/voters/`),
          bcrypt.hashSync(this.$svsAuth.user.id, saltRounds)
        );
        this.$data.hasvoted = true;

        let ballot_location =
          `realTimeVoting/pools/` +
          this.$data.pool[0] +
          "/" +
          this.$svsAuth.user.id;

        set(
          child(dbRef, ballot_location),
          JSON.parse(JSON.stringify(voteData.slice(1, voteData.length)))
        );
      }
    },
    poolpop: function (e) {
